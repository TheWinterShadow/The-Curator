...
from google.genai.types import HttpOptions, GenerateContentConfig, SpeechConfig, VoiceConfig, PrebuiltVoiceConfig
import anthropic
import time
import wave
...
class VertexClient:
    ...
    def synthesize_conversation(self, transcript: list[tuple[str, str]], speaker_map: dict[str, str], filename: str = "conversation.wav") -> str:
        """
        # --- Example Usage ---
        my_transcript = [
            ("Eli", "Curator, are the vault logs ready for review?"),
            ("System", "Affirmative. The logs have been decrypted and are currently being indexed for your terminal."),
        ...
        for i, (speaker, text) in enumerate(transcript):
            voice_name = speaker_map.get(
                speaker, "Charon")  # Default to Charon
            print(
                f"  [{i+1}/{len(transcript)}] Synthesizing {speaker} using {voice_name}..."
            )
