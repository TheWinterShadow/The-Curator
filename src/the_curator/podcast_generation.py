...
class PodcastGeneration:
    def __init__(self) -> None:
        self.vertex_client = VertexClient(
            project=os.environ.get("GOOGLE_CLOUD_PROJECT", "the-curator"),
    ...
    def generate_transcript(self, topic: str) -> list[tuple[str, str]]:
        response = self.vertex_client.generate_content(
            ...
            contents=(
                ...
                "The podcast features host Annabelle and co-host Link having a natural, informative conversation. "
                "They should explore the topic with genuine curiosity, banter, and energy — not just recite facts.\n\n"
                "Formatting rules (follow exactly):\n"
                "- Each line must start with exactly 'Annabelle:' or 'Link:' followed by a space and the spoken text.\n"
                "- No other formatting, headers, stage directions, or labels.\n\n"
                "Audio expression tags — embed these inline within the spoken text to add emotion, pacing, and texture:\n"
                ...
                "For example: 'Annabelle: [excitement] Okay so this is the part that blew my mind. [short pause] Ready?'\n\n"
